...
How **`apps/web`** wires Clerk OAuth discovery, the Streamable HTTP MCP endpoint, and tool handlers. For client setup, see [Model Context Protocol (MCP)](./mcp).
...
- [Model Context Protocol (MCP)](./mcp) — endpoints, tools table, client config
- [Deploy MCP on Vercel](../how-to/vercel-mcp-production) — production checklist
- [HTTP API (REST)](./rest-api) — same data model; session/cookie auth vs MCP OAuth

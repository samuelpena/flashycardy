...
export function StudySession({ deckUuid, cards, onSaveSession }: StudySessionProps) {
  ...
  useEffect(() => {
    ...
    setSaving(true);
    setSaveError(null);
    onSaveSession({ deckUuid, cardResults })
      .then((res) => {
        if (res?.error) {
  ...
  }, [completed, deck, results, deckUuid, onSaveSession, t]);
